...
#            self.vector_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
            with tf.variable_scope("vector_loss"):
                self.vector_loss = tf.nn.softmax_cross_entropy_with_logits(
                        logits=self.unary_scores, labels=self.input_label_indices_vector)
            ###
